...
export default async function handler(req,res){
  ...
  try{
    ...
    if(event.type==="invoice.payment_failed"){const n=await notifyPaymentFailed(obj);let grace=null;if(tenantId)grace=await markPaymentGrace(tenantId,{hours:24,payUrl:n.payUrl||null,invoiceId:String(obj.id||"")||null});await setContractBillingStatus({contractId,customerId,status:"payment_due"});await recordCustomerEvent({customerId,contractId,tenantId,eventType:"payment_failed",title:"Cobro pendiente · 24 h de cortesía",details:{invoice_id:obj.id,number:obj.number||null,amount_due:obj.amount_due,currency:obj.currency,pay_url:n.payUrl,email_sent:n.emailSent,whatsapp_sent:n.whatsappSent,grace_until:grace?.until||null}})}
