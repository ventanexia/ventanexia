...
import {activatePaid,suspendTenant,reactivateAgents,markPaymentGrace,db as entitlementDb} from "../../lib/entitlement.js";
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(event.type==="invoice.paid"){if(dealId)await hubspotPatch(dealId,{hs_next_step:"Servicio activo · revisar onboarding/entrega"});if(tenantId){await activatePaid(tenantId,{customerId:customerId||null,subscriptionId:String(obj.subscription||"")||null,invoiceId:String(obj.id||"")||null});await reactivateAgents(tenantId)}await setContractBillingStatus({contractId,customerId,status:"active"});const [mailSent,fiscalForwarded]=await Promise.all([sendPaidInvoiceEmail(obj).catch(()=>false),forwardFiscalInvoice(obj).catch(()=>false)]);await recordCustomerEvent({customerId,contractId,tenantId,eventType:"invoice_paid",title:"Cuota cobrada",details:{invoice_id:obj.id,number:obj.number||null,amount_paid:obj.amount_paid,currency:obj.currency,invoice_pdf:obj.invoice_pdf||null,mail_sent:mailSent,fiscal_forwarded:fiscalForwarded}})}

    if(event.type==="invoice.payment_failed"){const payUrl=String(obj.hosted_invoice_url||"").trim();let grace=null;if(tenantId)grace=await markPaymentGrace(tenantId,{hours:24,payUrl:payUrl||null,invoiceId:String(obj.id||"")||null});const n=grace?.reused?{payUrl,emailSent:false,whatsappSent:false}:await notifyPaymentFailed(obj);await setContractBillingStatus({contractId,customerId,status:"payment_due"});await recordCustomerEvent({customerId,contractId,tenantId,eventType:"payment_failed",title:grace?.reused?"Nuevo intento de cobro · continúa el mismo plazo":"Cobro pendiente · 24 h de cortesía",details:{invoice_id:obj.id,number:obj.number||null,amount_due:obj.amount_due,currency:obj.currency,pay_url:n.payUrl||payUrl,email_sent:n.emailSent,whatsapp_sent:n.whatsappSent,grace_until:grace?.until||null,grace_reused:Boolean(grace?.reused)}})}

    if(event.type==="customer.subscription.updated"){const status=String(obj.status||"");if(tenantId){if(["active","trialing"].includes(status)){await activatePaid(tenantId,{customerId:customerId||null,subscriptionId:subscriptionId||null,planKey:String(obj.metadata?.plan||"core"),featurePolicy:featurePolicyFromMeta(obj.metadata||{})});await reactivateAgents(tenantId);await setContractBillingStatus({contractId,customerId,status:"active"})}else if(status==="past_due"){await markPaymentGrace(tenantId,{hours:24});await setContractBillingStatus({contractId,customerId,status:"payment_due"})}else if(["unpaid","paused","incomplete_expired","canceled"].includes(status)){await suspendTenant(tenantId,`SUBSCRIPTION_${status.toUpperCase()}`);await setContractBillingStatus({contractId,customerId,status:"suspended"})}}await recordCustomerEvent({customerId,contractId,tenantId,eventType:"subscription_updated",title:"Suscripción actualizada",details:{subscription_id:obj.id,status}})}
