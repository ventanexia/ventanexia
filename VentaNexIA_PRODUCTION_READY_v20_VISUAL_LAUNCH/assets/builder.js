...
$("#builderForm").onsubmit=async e=>{
 ...
 try{
  ...
  lastRequestId=j.requestId||null; const b=j.blueprint||{};$("#outputEmpty").hidden=true;$("#outputReady").hidden=false;
  ...
  if(lastRequestId)$("#startTrial").hidden=false; $("#outputReady").scrollIntoView({behavior:"smooth",block:"nearest"});
...
if(trialBtn) trialBtn.onclick=async()=>{
 ...
 try{
  const r=await fetch("/api/start-trial",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({solutionId:lastRequestId,deviceId:getTrialDeviceId()})});
