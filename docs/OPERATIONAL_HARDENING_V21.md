...
## Hobby deployment compatibility

For Vercel Hobby compatibility, API endpoints are dispatched through a single Vercel Function (`api/router.js`). Public endpoint URLs remain unchanged (`/api/<endpoint>`). Route implementations live in `server/handlers/`. Stripe webhook requests bypass router body parsing so signature verification still receives the raw request stream.
