...
need(master,/const hit=portalSalesQtyForProduct\(p,sales\)/,'sales must be matched through the normalized SKU/EAN/name resolver');
need(master,/soldWindow:Number\(hit\.qty\|\|0\)/,'matched sold units must be exposed per product');
need(master,/noSalesData:!hit\.match/,'no-history must be distinguished from zero units after normalized matching');
