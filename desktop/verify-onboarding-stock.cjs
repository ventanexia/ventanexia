...
need(backend,/function autoUrgentDays\(targetDays\)/,'urgent threshold must adapt to the configured coverage target');
...
need(preload,/stockImportFile:\(options=\{\}\)=>ipcRenderer\.invoke\('stock:import-file',options\)/,'preload must expose policy-aware stock import');
...
need(renderer,/headers:\['sku','ean','fabricante','producto','stock_actual','ventas_periodo','media_diaria','dias_cobertura','cantidad_a_pedir','estado'\]/,'manufacturer, SKU and EAN must remain separate in import exports');
...
need(agentHome,/autoUrgentDaysForPolicy/,'agent home stock copy must use the active urgent threshold');
