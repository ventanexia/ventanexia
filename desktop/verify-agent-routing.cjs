...
assert.equal(AGENT_CATALOG.length,14,'El catálogo debe incluir los 14 especialistas actuales, incluido Dirección');
assert.equal(AGENT_CATALOG.some(a=>a.key==='direction'&&/Dirección/.test(a.name||'')),true,'Dirección debe existir como agente especialista privado');
