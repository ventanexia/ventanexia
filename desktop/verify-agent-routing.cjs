...
assert.equal(AGENT_CATALOG.some(a=>a.key==='direction'),false,'Dirección debe quedar fuera del catálogo genérico y solo abrirse tras PIN');
...
const handlerCount=(main.match(/ipcMain\.handle\('chat:send'/g)||[]).length+(master.match(/ipcMain\.handle\('chat:send'/g)||[]).length+(entry.match(/ipcMain\.handle\('chat:send'/g)||[]).length;
...
assert.doesNotMatch(master,/ipcMain\.handle\('chat:send'/,'master.cjs debe exportar el handler, no registrarlo');
...
assert.match(entry,/ipcMain\.handle\('chat:send'/,'master-entry.cjs debe registrar el único router chat:send');
assert.doesNotMatch(entry,/originalHandle\('chat:send'/,'No debe existir monkeypatch de ipcMain.handle');
assert.match(master,/module\.exports=\{chatSendHandler\}/,'master.cjs debe exportar chatSendHandler');
...
assert.match(master,/ipcMain\.handle\('email:action'/,'Debe existir un handler seguro para acciones Gmail');
assert.match(master,/messages\/send/,'Debe poder enviar una respuesta solo tras elección explícita');
assert.match(master,/drafts/,'Debe poder crear borradores reales de Gmail');
assert.match(master,/messages\/.*\/trash/,'Debe poder mover correos a la papelera');
assert.match(master,/removeLabelIds:\['INBOX'\]/,'Debe poder archivar correos');
assert.match(master,/addLabelIds:\['STARRED'\]/,'Debe poder destacar correos');
assert.match(master,/addLabelIds:\['IMPORTANT'\]/,'Debe poder marcar importantes');
assert.match(master,/chooseEmailTarget/,'Debe localizar el correo concreto antes de actuar');
