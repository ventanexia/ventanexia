...
export function jevModel(){return String(process.env.JEV_MODEL||"").trim()}

let cachedModel={name:"",at:0};
async function resolveModel(token){
  const configured=jevModel();if(configured)return configured;
  if(cachedModel.name&&(Date.now()-cachedModel.at)<3600000)return cachedModel.name;
  const r=await fetch("https://api.typesafe.ai/v1/models",{headers:{Authorization:"Bearer "+token,Accept:"application/json"}});
  const data=await r.json().catch(()=>null);
  if(!r.ok)throw new Error(String(data?.error?.message||data?.error||data?.message||("JEV_MODELS_HTTP_"+r.status)).slice(0,400));
  const list=Array.isArray(data)?data:Array.isArray(data?.models)?data.models:Array.isArray(data?.data)?data.data:[];
  const names=list.map(x=>typeof x==="string"?x:String(x?.id||x?.name||x?.model||"")).filter(Boolean);
  const picked=names.find(x=>/jev/i.test(x))||names[0];
  if(!picked)throw new Error("JEV_NO_MODEL_AVAILABLE");
  cachedModel={name:picked,at:Date.now()};return picked;
}
...
export async function createJevDecision({state,questions,model="",timeoutMs=8000}={}){
  const token=key();if(!token)throw new Error("JEV_NOT_CONFIGURED");
  validateQuestions(questions);
  const selectedModel=String(model||"").trim()||await resolveModel(token);
  ...
  try{
    const r=await fetch(ENDPOINT,{
      ...
      body:JSON.stringify({model:selectedModel,state,questions}),
    ...
    return {ok:true,model:data?.model||selectedModel,answers,usage:data?.usage||null};
  }catch(e){
