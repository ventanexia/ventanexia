...
export async function markPaymentGrace(tenantId,{hours=24,payUrl=null,invoiceId=null}={}){
  ...
  const failedAt=new Date(),until=new Date(failedAt.getTime()+Math.max(1,Number(hours)||24)*60*60*1000);
  await db(`vnx_entitlements?tenant_id=eq.${encodeURIComponent(tenantId)}`,{method:"PATCH",body:JSON.stringify({
    ...
    feature_policy:{...policy,billing_status:"payment_due",payment_failed_at:failedAt.toISOString(),payment_grace_until:until.toISOString(),payment_url:payUrl||null,payment_invoice_id:invoiceId||null}
  })});
