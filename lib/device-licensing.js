...
function publicSummary(summary){
  return {
    ...
    extraDeviceCount:summary.extraDeviceCount,
    limit:summary.limit,
    activeCount:summary.activeCount,
    available:summary.available,
    extraDeviceMonthlyEur:summary.extraDeviceMonthlyEur
  };
}
...
export async function registerDevice({customerCode,activationCode,deviceKey,fingerprintHash,deviceName,platform,appVersion}){
  ...
  if(current){
    ...
    return {ok:true,registered:false,customerId:tenant.customer_code,deviceId:current.id,...publicSummary(summary)};
  }
  ...
  const summary=await getDeviceSummary(tenant.id);
  return {ok:true,registered:true,customerId:tenant.customer_code,deviceId:device?.id||null,...publicSummary(summary)};
}
...
export async function touchDevice({customerCode,activationCode,deviceKey}){
  ...
  const ent=await getEntitlement(tenant.id);
  const access=computeAccess(ent,"dashboard");
  if(!access.allowed)return {ok:false,code:"LICENSE_NOT_ACTIVE",state:access.state,reason:access.reason};
  const rows=await db(`vnx_devices?tenant_id=eq.${encodeURIComponent(tenant.id)}&device_key=eq.${encodeURIComponent(clean(deviceKey,120))}&select=id,status`);
  const device=rows?.[0];
  ...
  const summary=await getDeviceSummary(tenant.id);
  return {ok:true,customerId:tenant.customer_code,deviceId:device.id,...publicSummary(summary)};
}
