...
export function authMode(){
  return String(process.env.CHAT_AUTH_MODE||"log").trim().toLowerCase()==="enforce"?"enforce":"log";
}
...
export async function consumeMeter(license,meter,quantity=1,metadata={}){
  ...
  try{
    ...
    // Fallo de infraestructura del contador: no bloqueamos al cliente, pero queda registrado.
    return {ok:true,meterError:String(e?.message||e).slice(0,160)};
  }
}
