...
export async function meterStatus(license,meter){
  if(isMasterPlan(license?.planKey))return {ok:true,unlimited:true,remaining:Number.MAX_SAFE_INTEGER,monthlyLimit:Number.MAX_SAFE_INTEGER,usedThisMonth:0};
  try{
    const out=await rpc("vnx_meter_status",{p_customer_code:license.customerId,p_meter:meter});
    if(out?.ok)return {...out,ok:true};
    return {ok:false,limit:true,code:out?.code||"USAGE_LIMIT_REACHED",message:out?.error||out?.message||null,remaining:Number(out?.remaining||0),monthlyLimit:Number(out?.monthlyLimit||0),usedThisMonth:Number(out?.usedThisMonth||0)};
  }catch(e){
    return {ok:true,infrastructure:true,meterError:String(e?.message||e).slice(0,160),remaining:null,monthlyLimit:null,usedThisMonth:null};
  }
}
